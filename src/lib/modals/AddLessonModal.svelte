...
{#if open}
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
    <div class="bg-white rounded-2xl p-10 w-full max-w-3xl relative border-2 border-black shadow-lg flex flex-col items-stretch min-w-[600px] min-h-[500px]">
      ...
      <h3 class="text-2xl font-bold text-center mb-6 text-primary-700">{editMode ? '编辑 Lesson' : '添加 Lesson'}{#if sectionTitle} <span class="section-name">({sectionTitle})</span>{/if}</h3>
      <div class="flex flex-col gap-3 mb-4">
        ...
        <textarea
          ...
          maxlength="10000"
        />
        <div class="text-xs text-gray-500 text-right">{localContent.length}/10000</div>
      </div>
      {#if error}
      ...
      <div class="flex gap-3 justify-end mt-2">
        {#if editMode}
          <button type="button" class="btn-gradient flex items-center gap-2 px-6 py-2 font-semibold rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 w-full justify-center mt-2 disabled:opacity-60" on:click={handleSave} disabled={loading}>
            {loading ? "保存中..." : "保存"}
          </button>
        {:else}
          <button type="button" class="btn-gradient flex items-center gap-2 px-6 py-2 font-semibold rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 w-full justify-center mt-2 disabled:opacity-60" on:click={handleSave} disabled={loading}>
